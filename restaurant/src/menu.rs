...
pub trait Repository {
    fn get_all(&self) -> impl Future<Output = RepoResult<Vec<MenuItem>>> + Send;
    fn get(&self, id: u32) -> impl Future<Output = RepoResult<MenuItem>> + Send;

    fn create(&mut self, item: MenuItem) -> impl Future<Output = RepoResult<MenuItem>> + Send;
    fn remove(&mut self, item: MenuItem) -> impl Future<Output = RepoResult<()>> + Send;
    fn update(&mut self, item: MenuItem) -> impl Future<Output = RepoResult<MenuItem>> + Send;
}

pub async fn get<T: Repository>(repo: &T) -> Result<Vec<MenuItem>> {
...
impl MenuItem {
    ...
    pub async fn set_cook_time<T: Repository>(
        mut self,
        repo: &mut T,
        d: TimeDelta,
    ) -> Result<MenuItem> {
